...
$('tableBody').addEventListener('change', (e) => {
    if (e.target.classList.contains('hide-cb') && e.target.checked) {
...
function calculateReconcile(parsed) {
    let totalQR = 0, totalEDC = 0, totalBiaya = 0;
    let lastBalance = "0";
    
    const processed = parsed.map(t => {
        ...
        if (t.type === 'DB' && isMinor && !isQR) {
            ...
            if (isQR) totalQR += t.numericAmount;
            else if (isEDC) totalEDC += t.numericAmount;
        }
    ...
    return { 
        processed, 
        summary: { totalQR, totalEDC, totalBiaya, lastBalance, totalAdminQR, isExactQR, totalAdminEDC, isExactEDC }
    };
}

function processData() {
    const rawText = $('rawInput').value;
    ReconcileStore.transactions = [];

    if (!rawText.trim()) {
        $('tableBody').innerHTML = EMPTY_STATE_HTML;
    ...
    if (!parsed.length) {
        $('tableBody').innerHTML = ERROR_STATE_HTML;
        $('summaryCards').style.display = 'none';
        $('filterContainer').style.display = 'none';
        return;
    }
    ...
    const { processed, summary } = calculateReconcile(day1Parsed);
    ReconcileStore.transactions = processed;

    $('summaryCards').style.display = 'grid';
    $('filterContainer').style.display = 'flex';
    const finalBalanceNum = parseToNumber(summary.lastBalance);
    setResult('finalBalanceDisplay', 'copyFinalBalance', formatCurrency(finalBalanceNum));
    setResult('totalBiayaAdminDisplay', 'copyTotalBiayaAdmin', formatCurrency(summary.totalBiaya));
    setResult('totalQrDisplay', 'copyTotalQr', formatCurrency(summary.totalQR));
    setResult('totalEdcDisplay', 'copyTotalEdc', formatCurrency(summary.totalEDC));

    if (!summary.isExactQR) {
        ...
        setResult('totalAdminQrDisplay', 'copyTotalAdminQr', formatCurrency(summary.totalAdminQR));
    }

    if (!summary.isExactEDC) {
        ...
        setResult('totalAdminEdcDisplay', 'copyTotalAdminEdc', formatCurrency(summary.totalAdminEDC));
    }
    renderTable();
}

function renderReconcileRowHTML(t) {
    return `
        <tr ${t.isMinor ? 'class="is-minor"' : ''}>
            ...
            <td class="hide-cb-wrapper">
                <input type="checkbox" class="hide-cb" title="Sembunyikan">
            </td>
            <td class="col-center">
...
function renderTable() {
    if (!ReconcileStore.transactions.length) return;
    
    let data = [...ReconcileStore.transactions];
    ...
    $('tableBody').innerHTML = data.map(renderReconcileRowHTML).join('');
}
